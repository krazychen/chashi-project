...
import com.io.yy.util.callback.MethodCallback;
import com.io.yy.util.collect.ListUtils;
import com.io.yy.util.collect.SetUtils;
import com.io.yy.util.excel.annotation.ExcelField;
import com.io.yy.util.excel.annotation.ExcelField.Type;
import com.io.yy.util.excel.annotation.ExcelFields;
			...
			for (int j = 0; j < annotationList.size(); j++){//Object[] os : annotationList){
				...
				if (val != null){
					...
					if (StringUtils.isNotBlank(ef.dictType())){
						try{
							SysDictDataRedisService sdrs = (SysDictDataRedisService) SpringContextUtil.getBean("com.io.yy.system.service.impl.SysDictDataRedisServiceImpl");
							val = sdrs.getSysDictDataValue(ef.dictType(),val.toString());
	//							Class<?> dictUtils = Class.forName("com.io.yy.util.DictUtils");
