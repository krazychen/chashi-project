...
public class JwtRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        for(SysRole sr:loginSysUserRedisVo.getSysRoleList()){
            roles.add(sr.getCode());
        }
        authorizationInfo.setRoles(roles);
        // 设置权限
        Set<String> permissions=new HashSet<String>();
        for(SysMenuQueryVo sm:loginSysUserRedisVo.getSysMenuList()){
            permissions.addAll(listPermissions(sm));
        }
     ...
     * @return
     */
    public static Set<String> listPermissions(SysMenuQueryVo sm) {
        Set<String> permissions = new HashSet<>();
        if (sm.getChildren()!=null&&sm.getChildren().size()!=0) {
            permissions.add(sm.getPermission());
            for (SysMenuQueryVo smchild : sm.getChildren()) {
                permissions.addAll(listPermissions(smchild));
            }
        } else {
            permissions.add(sm.getPermission());
        }
